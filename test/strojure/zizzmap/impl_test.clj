...
(def ^:private -e (impl/boxed-map-entry :k (impl/boxed-value (identity :x))))
...
(deftest boxed-map-entry-t
  (test/are [expr result] (= result expr)
    ...
    (assoc -e 0 :kk) #_= [:kk :x]
    (assoc -e 1 :xx) #_= [:k :xx]
    (assoc -e 1 (impl/boxed-value (identity :xx))) #_= [:k :xx]
    (assoc -e 2 :y) #_= [:k :x :y]
    ...
    (let [a (atom :pending)
          e (impl/boxed-map-entry :k (impl/boxed-value (do (reset! a :realized)
                                                           :x)))
    ...
    (.assocN ^IPersistentVector -e 1 (impl/boxed-value (identity :xx))) #_= [:k :xx]
    ...
    (.equiv ^IPersistentVector -e (impl/boxed-map-entry :k (impl/boxed-value (identity :x)))) #_= true
...
(def ^:private -m
  (impl/persistent-map {:a (impl/boxed-value (identity :x))
                        :b :y}))
...
(deftest persistent-map-t
  (test/are [expr result] (= result expr)
    ...
    (= {:a :x :b :y} -m) #_= true
    (= (impl/persistent-map {:a (impl/boxed-value (identity :x))})
       (impl/persistent-map {:a (impl/boxed-value (identity :x))})) #_= true
    ...
    (conj -m [:c (impl/boxed-value (identity :z))]) #_= {:a :x, :b :y, :c :z}
  ...
  (testing "Value laziness"
    (test/are [expr result] (= result expr)
      ...
      (let [a (atom :pending) m (impl/persistent-map {:a (impl/boxed-value
                                                           (do (reset! a :realized)
                                                               :x))})
            before @a
            x (get m :a)
